...
public class CalTrafficFlowThread implements Runnable{
    ...
    public CalTrafficFlowThread(String fromdb, String fromcol, String todb, String tocol, long startTime, long timespan, long frequency) {
        this.mongoClient = new MongodbClient().getMongoClient();
        this.fromdb = mongoClient.getDatabase(fromdb);
        this.fromcol = this.fromdb.getCollection(fromcol);

        this.todb = mongoClient.getDatabase(todb);
        this.tocol = this.todb.getCollection(tocol);
    ...
    public void run() {
        ...
        while(true){
            ...
            while(cursor.hasNext()) {
                item_doc = cursor.next();
//                System.out.println(item_doc.toJson());
                dict.put(item_doc.get("_id").toString(), (Integer) item_doc.get("trafficFlow"));
            }
...
//                    System.out.println(dict.get(key));
    ...
    public static void main(String[] args) {
        CalTrafficFlowThread Ctf = new CalTrafficFlowThread("EntityModel", "GantryRecord", "DigitalModel","GantryDigital",1635948000000L, 3600000, 30000);

//        Thread Thrd = new Thread(Ctf);
//        Thrd.start();
