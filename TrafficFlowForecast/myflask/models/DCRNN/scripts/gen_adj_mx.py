...
def get_adjacency_matrix_new(distance_df, sensor_ids, normalized_k=0.1):
    """
...
if __name__ == '__main__':
    ...
    distance_df = pd.read_csv(args.distances_filename, index_col=0)
    normalized_k = args.normalized_k
    _, sensor_id_to_ind, adj_mx = get_adjacency_matrix_new(distance_df, sensor_ids, normalized_k)
    # Save to pickle file.
    with open(args.output_pkl_filename, 'wb') as f:
        pickle.dump([sensor_ids, sensor_id_to_ind, adj_mx], f, protocol=2)
    f=open('data/model/dcrnn_{}.yaml'.format(args.configname), 'r')
    config=yaml.load(f)
    f.close()
    with open('data/model/dcrnn_{}.yaml'.format(args.configname), 'w') as f:
        config['data']['graph_pkl_filename']=args.output_pkl_filename
