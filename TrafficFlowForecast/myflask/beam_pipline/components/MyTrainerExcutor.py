...
from myflask.beam_pipline.components  import fn_args_utils
...
from myflask.utils.util import copy_dir
...
class Executor(GenericExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    #print(fn_args.serving_model_dir,fn_args.eval_model_dir)
    #assert 0>1
    ...
    training_spec['estimator'].train(input_fn=training_spec['train_spec'].input_fn,max_steps=training_spec['train_spec'].max_steps,
    hooks=training_spec['train_spec'].hooks
    )
    #logging.logger.warning(training_spec['estimator'].get_variable_value('signal_early_stopping/STOP')) 
    #logging.logger.warning(training_spec['estimator'].get_variable_names()) 
    assets_extra={
      'adj_mx.pkl':os.path.join(fn_args.transformed_examples_dir,'adj_mx.pkl'),
      'hex_id.yaml':os.path.join(fn_args.transformed_examples_dir,'hex_id.yaml'),
      'sensor_ids.yaml':os.path.join(fn_args.transformed_examples_dir,'sensor_ids.yaml'),
      'model_final.yaml':os.path.join(fn_args.transformed_examples_dir,'model_final.yaml')
    }
    training_spec['estimator'].export_saved_model(
      os.path.join(fn_args.serving_model_dir,'export'), training_spec['serving_receiver_fn'],
    ...
    if _is_chief():
      ...
      copy_dir(os.path.join(fn_args.serving_model_dir,'export'),serving_dest)
