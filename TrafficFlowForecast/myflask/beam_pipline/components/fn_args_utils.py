...
def get_common_fn_args(input_dict: Dict[Text, List[types.Artifact]],
  ...
  if input_dict.get(constants.SCHEMA_KEY):
    schema_type = artifact_utils.get_single_instance(input_dict[constants.SCHEMA_KEY]).get_string_custom_property('type')
  else:
    schema_type = 'float32'
  ...
  transformed_examples_dir=artifact_utils.get_single_instance(input_dict[constants.EXAMPLES_KEY]).uri
  ...
  return FnArgs(
      ...
      transformed_examples_dir=transformed_examples_dir,
