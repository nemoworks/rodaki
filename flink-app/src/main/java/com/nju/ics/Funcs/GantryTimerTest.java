...
public class GantryTimerTest {
        public static void main(String[] args) throws Exception {
                ...
                DataStream<JSONObject> rawGantry = env.readFile(csvGantryInput, gantrycsv)
                                ...
                                .map(new Row2JSONObject(new String[] { "GANTRYHEX", "GANTRYID", "TRANSTIME", "VLP",
                                                "VLPC", "VEHICLETYPE" }));

                DataStream<GantryRecordSimple> gantryRecordSimple = rawGantry
                                .process(new ProcessFunction<JSONObject, GantryRecordSimple>() {

                                        @Override
                                        public void processElement(JSONObject value,
                                                        ProcessFunction<JSONObject, GantryRecordSimple>.Context ctx,
                                                        Collector<GantryRecordSimple> out) throws Exception {
                                                // TODO Auto-generated method stub
                                                value.put(DataSourceJudge.timeKey, ctx.timestamp());
                                                // Thread.sleep(1); // 延时1毫秒
                                                out.collect(JSON.toJavaObject(value, GantryRecordSimple.class));
                                        }
        ...
        static class timestampAssigner implements SerializableTimestampAssigner<Row> {
                ...
                public long extractTimestamp(Row element, long recordTimestamp) {
                        ...
                        try {
                                timestamp = time.parse(element.getFieldAs(2)).getTime();
                        } catch (Exception e) {
